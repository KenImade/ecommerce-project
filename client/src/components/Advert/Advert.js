import React from "react";
import "./assets/Advert.css";
import img from "../../assets/shared/desktop/image-best-gear.jpg";

const Advert = () => {

  return (
    <section className='advert'>
        <div>
            <h2>Bringing you the <span className="styledH2">best</span> audio gear</h2>
            <p className="body">
                Located at the heart of New York City,
                Audiophile is the premier store for high end headphones, 
                earphones, speakers, and audio accessories. 
                We have a large showroom and luxury demonstration rooms 
                available for you to browse and experience a wide range 
                of our products. Stop by our store to meet some of the 
                fantastic people who make Audiophile the best place to buy 
                your portable audio equipment.
            </p>
        </div>

        <img src={img} alt="man with headphones"/>
    </section>
  )
}

export default Advert